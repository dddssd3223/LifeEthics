"""LIFE_ETHICS 최종 이관 패키지 V3 export (분석·재판정 없음, V1/V2 보존, 결정적 출력).

선정: data/phase2/LIFE_ETHICS/final_logic_correction/final_content_asset_index_v3.jsonl
원문: Phase 1 data/questions/json/*.json, data/questions/images/*.png (읽기 전용)
출력: FINAL_TRANSFER/LIFE_ETHICS_V3/{manifest.json, README.md, questions.jsonl, questions.xlsx, images/}
      FINAL_TRANSFER/LIFE_ETHICS_FINAL_TRANSFER_V3.zip (루트 LIFE_ETHICS_V3/)
...
CORR = P2 / "final_logic_correction"
INDEX = CORR / "final_content_asset_index_v3.jsonl"
BASE = ROOT / "FINAL_TRANSFER"
OUT = BASE / "LIFE_ETHICS_V3"
ZIP = BASE / "LIFE_ETHICS_FINAL_TRANSFER_V3.zip"
...
def main():
    ...
    corr = [json.loads(l) for l in open(CORR / "final_logic_correction.jsonl", encoding="utf-8")]
    corr_drop = {r["question_id"] for r in corr if r["final_decision"] == "DROP"}
    n = len(corr) - len(corr_drop)
    ...
    for sel in sorted(idx, key=lambda r: r["question_id"]):
        ...
        rows.append({
            ...
            "logic_correction": sel["logic_correction"],
    ...
    head = ["question_id", "exam_year", "exam_session", "question_number", "official_answer", "points",
            ...
            "logic_correction", "adaptation_required", "image_filename"]
    ws.append(head)
    for r in rows:
        ...
        ws.append([r["question_id"], r["exam_year"], r["exam_session"], r["question_number"], r["official_answer"],
                   ...
                   f"{link['sample_question']} ({link['match_depth']})" if link else None,
                   r["logic_correction"], r["adaptation_required"], r["image"]])
    ...
    summ = json.load(open(CORR / "final_logic_correction_summary.json", encoding="utf-8"))
    manifest = {
        "package": "LIFE_ETHICS_FINAL_TRANSFER_V3",
        "subject": "LIFE_ETHICS",
        "selection_source": "data/phase2/LIFE_ETHICS/final_logic_correction/final_content_asset_index_v3.jsonl",
        ...
        "selection_history": {
            ...
            "final_v2": summ["base"]["previous_final"],
            "logic_correction_targeted": summ["targeted_review_candidates"],
            "logic_correction_drop": summ["DROP_after_targeted_review"],
            "final_v3": len(jl),
        },
        "previous_package": "FINAL_TRANSFER/LIFE_ETHICS_V2 (historical snapshot, freeze 99bc4a6)",
    ...
    (OUT / "README.md").write_text(
        ...
        f"- 선정 기준: V2 136문항에서 정답 결정 논리가 생활과 윤리 고유 사상가 비교·세부 이론인 문항 "
        f"{summ['DROP_after_targeted_review']}개를 제거(answer-logic correction).\n"
    ...
    with zipfile.ZipFile(ZIP) as z:
        bad = z.testzip()
        names = z.namelist()
        zimgs = [x for x in names if x.startswith("LIFE_ETHICS_V3/images/") and x.endswith(".png")]
        zqa = {
            "corrupt_files": 0 if bad is None else 1,
            "root_dirs": sorted({x.split("/")[0] for x in names}),
            "required_files": all(f"LIFE_ETHICS_V3/{f}" in names for f in ("questions.jsonl", "questions.xlsx", "manifest.json", "README.md")),
            "images_count": len(zimgs),
            "jsonl_records": sum(1 for l in z.read("LIFE_ETHICS_V3/questions.jsonl").decode("utf-8").splitlines() if l.strip()),
            "export_to_zip_hash_mismatch": sum(sha256(z.read(x)) != src_hash[Path(x).stem] for x in zimgs),
            "entries": len(names),
        }
    if not (zqa["corrupt_files"] == 0 and zqa["root_dirs"] == ["LIFE_ETHICS_V3"] and zqa["required_files"]
